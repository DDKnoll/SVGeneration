...
SVGeneration.Editor = React.createClass({displayName: "Editor",
  ...
  dataURI: function(){
    return "url(data:image/svg+xml;utf8," + this.generate() + ")";
  },
  renderSVG: function(){
    return "<div class='svg-background' style='background-image:"+this.dataURI()+";'></div>"
  },
  loadData: function(){
    $.ajax({
  ...
  render: function() {
    ...
    var style = (this.state.image && this.state.script
                  ? {paddingTop: topPadding, backgroundImage: this.dataURI(), opacity: 1}
                  : {paddingTop: topPadding, opacity: 0}
                );
    console.log(style);
    return (
      React.createElement("div", {className: "editor", style: style}, 
        React.createElement("div", {dangerouslySetInnerHTML: {__html: this.renderSVG()}}), 
